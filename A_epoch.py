...
trainer = Trainer(max_epochs=MAX_EPOCHS, accelerator="gpu", devices=DEVICES, logger=logger, callbacks=[lr_monitor, early_stopping, checkpoint_callback], deterministic=True)
trainer.fit(model, datamodule=data)
trainer.save_checkpoint("./saved_models/{}.ckpt".format(name))               

for name in os.listdir("./saved_models/"):
    if "EPOCH_" in name:
        new_name = name.replace("=", "_").replace(".ckpt.", ".")
        print(name, new_name)
        os.rename("./saved_models/{}".format(name), "./saved_models/{}".format(new_name))
